...
function main(): void {
  ...
  const soldierDefBg = buildFullDeck().find((c: any) => c.id === 'soldier_0'); // 士兵 value=1 cost=1
  const general3Def = buildFullDeck().find((c: any) => c.category === 'general' && c.value >= 3);
  const atk3Def = general3Def || soldierDefBg;
  const baguaDef = buildFullDeck().find((c: any) => c.id === 'bagua_0');
  const leatherDef = buildFullDeck().find((c: any) => c.id === 'leather_0'); // 皮甲 value=1
  // 构造一手牌：
  // A(fpBg): 武将(3攻) + 皮甲 + 八卦阵
  // B(defBg): 皮甲 + 八卦阵
  ...
  let rBg = applyCardEffect(engineBg, { uid: 'ga', def: atk3Def }, fpBg);
  assert(rBg.ok, 'A 打出武将攻击 B');
  assert(engineBg.turn.isAwaitingDefense(), '进入防御响应');
  // B 先出皮甲抵消 1
  rBg = applyCardEffect(engineBg, { uid: 'lb', def: leatherDef }, defBg);
  assert(rBg.ok, 'B 先出皮甲 +1 防御池');
  assert(engineBg.defensePool === 1, 'B 防御池 = 1');
  // B 再出八卦阵 → 立即结算反弹 (伤害 - 防御池)
  rBg = applyCardEffect(engineBg, { uid: 'bgb', def: baguaDef }, defBg);
  assert(rBg.ok, 'B 打出八卦阵（皮甲后）');
  assert(rBg.triggeredReflect === true, '八卦阵生效触发反弹');
  const expectReflectDmg = Math.max(0, atk3Dmg - 1);
  assert(engineBg.pendingAttack?.damage === expectReflectDmg,
    `反弹伤害 = 原伤害 ${atk3Dmg} - 皮甲1 = ${expectReflectDmg}，实际 ${engineBg.pendingAttack?.damage}`);
  ...
  rBg = applyCardEffect(engineBg, { uid: 'bga', def: baguaDef }, fpBg);
  ...
  assert(engineBg.pendingAttack?.damage === expectReflect2B,
    `嵌套反弹伤害 = ${reflectDmg2A} - A皮甲1 = ${expectReflect2B}，实际 ${engineBg.pendingAttack?.damage}`);
  ...
  assert(hpAfterB === Math.max(0, hpBeforeB - expectReflect2B),
    `B 实际扣血 ${hpBeforeB - hpAfterB}，应扣 ${expectReflect2B}（hp ${hpAfterB}）`);
  ...
  if (general3def) {
    ...
    const healDef = bfd3().find((c: any) => c.id === 'medicine_0');
