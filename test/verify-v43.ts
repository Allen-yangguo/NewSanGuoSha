...
import { CardCategory, CardInstance, PouchType, TurnPhase } from '../assets/scripts/core/types';
import { applyCardEffect } from '../assets/scripts/core/CardEffect';
import { buildFullDeck, buildLimitedCards } from '../assets/scripts/core/cards';
...
function defById(id: string): any {
  return DECK.find(d => d.id.startsWith(id)) || LIMITED.find(d => d.id === id);
}
function inst(id: string, uid: string): CardInstance {
...
function main(): void {
  console.log('=== 智者牌：打出获得锦囊标记 ===');
  {
    ...
    const r = applyCardEffect(e, inst('zhuge', 't_zhuge'), fp);
  ...
  }
  {
    ...
    applyCardEffect(e, inst('zhouyu', 't_zhouyu'), 0);
  ...
  }
  {
    ...
    applyCardEffect(e, inst('simayi', 't_simayi'), 0);
    const p = e.state.players[0];
  ...
  console.log('\n=== 缺锦囊（缺血 hp=2 / 残血 hp=1 可用）===');
  {
    ...
    const r1 = e.usePouch(fp, 'zhuge', PouchType.Que, 'qimen');
    assert(!r1.ok, '血量正常时缺锦囊不可用');
    e.state.players[fp].hp = 2;
    const r2 = e.usePouch(fp, 'zhuge', PouchType.Que, 'qimen');
  ...
  console.log('\n=== 锦囊随机出牌（choice 空 → 系统随机）===');
  {
    ...
    e.state.players[fp].pouches.zhuge = { que: true, can: false, ji: false };
    e.state.players[fp].hp = 2;
    const r = e.usePouch(fp, 'zhuge', PouchType.Que, '');
    ...
    e.state.players[fp].hp = 1;
    const r2 = e.usePouch(fp, 'zhuge', PouchType.Can, '');
  ...
  console.log('\n=== 残锦囊（残血 hp=1 可用）===');
  {
    ...
    e.state.players[fp].hp = 1;
    const r = e.usePouch(fp, 'zhuge', PouchType.Can, 'zhangfei_shemiao');
    assert(r.ok, '残锦囊可用');
  ...
  console.log('\n=== 大乔 / 孙尚香（周瑜残锦囊，打出触发）===');
  {
    ...
    const r = e.usePouch(fp, 'zhouyu', PouchType.Can, 'daqiao');
    assert(r.ok, '周瑜残锦囊产出大乔');
    // 打出大乔
    e.state.players[fp].hand.push(inst('daqiao', 't_daqiao'));
    const r2 = applyCardEffect(e, inst('daqiao', 't_daqiao'), fp);
    assert(r2.ok, '大乔可打出');
  ...
  }
  {
    ...
    const r = e.usePouch(fp, 'zhouyu', PouchType.Can, 'sunshangxiang');
    assert(r.ok, '孙尚香入手牌');
    const r2 = applyCardEffect(e, inst('sunshangxiang', 't_ssx'), fp);
    ...
    const r4 = applyCardEffect(e3, inst('sunshangxiang', 't3_ssx'), 0);
  ...
  console.log('\n=== 司马懿：坚壁清野 2 层龟背阵 ===');
  {
    ...
    e.state.players[fp].hp = 2;
    const r = e.usePouch(fp, 'simayi', PouchType.Que, 'jianbi');
    assert(r.ok, '坚壁清野入手牌');
    // 打出坚壁清野
    e.state.players[fp].hand.push(inst('jianbi', 't_jianbi'));
    const r2 = applyCardEffect(e, inst('jianbi', 't_jianbi'), fp);
    assert(r2.ok, '坚壁清野打出');
    assert(e.guiBeiLayers === 2, `龟背减攻 2 层（实际 ${e.guiBeiLayers}）`);
    // 再打龟背阵 +1 层
    e.state.players[fp].hand.push(inst('guibei', 't_guibei'));
    const r3 = applyCardEffect(e, inst('guibei', 't_guibei'), fp);
  ...
  console.log('\n=== 龟背阵减伤生效 ===');
  {
    ...
    applyCardEffect(e, inst('soldier', 't_sol'), fp);
    e.defenderPass();
    // 士兵攻1 - 龟背3层 = 0
    ...
    applyCardEffect(e, inst('yitianjian', 't_yj'), fp);
  ...
  console.log('\n=== 鱼鳞阵：己方防具防御 +1 ===');
  {
    ...
    const r = applyCardEffect(e, inst('yulin', 't_yulin'), fp);
    ...
    applyCardEffect(e, inst('soldier', 't_sol2'), enemy);
    e.state.players[fp].hand.push(inst('leather', 't_lea'));
    const r2 = applyCardEffect(e, inst('leather', 't_lea'), fp);
  ...
  console.log('\n=== 盾系列防具 ===');
  {
    ...
    applyCardEffect(e, inst('soldier', 't_sol3'), enemy);
    e.state.players[fp].hand.push(inst('wood_shield', 't_ws'));
    const r = applyCardEffect(e, inst('wood_shield', 't_ws'), fp);
  ...
  console.log('\n=== 奇门遁甲 / 火烧连营：兵法 +3 ===');
  {
    ...
    const r = applyCardEffect(e, inst('qimen', 't_qm'), fp);
    assert(r.ok, '奇门遁甲打出');
    ...
    applyCardEffect(e, inst('huoshao', 't_hs'), fp);
  ...
  console.log('\n=== 急锦囊：普通攻击致死 → 丹药保 1 血 ===');
  {
    ...
    const r = e.usePouch(fp, 'zhuge', PouchType.Ji, 'huanhun_dan');
    ...
    const r2 = applyCardEffect(e, dan, fp);
  ...
  }
  {
    ...
    Math.random = origRandom;
    assert(r.ok && (r as any).saved === true, `使用急锦囊 · 抽中绝疗丹（saved=${(r as any).saved}）`);
    ...
    assert(settled.ok && (settled as any).saved === true, '结算 · 绝疗丹保 1 血');
  ...
  console.log('\n=== 回合结束清桌 ===');
  {
    ...
    applyCardEffect(e, inst('ration', 't_ration'), fp);
  ...
  console.log('\n=== 锦囊选项查询 ===');
  {
    ...
    assert(opts.some(o => o.pouch === PouchType.Que && o.choices.length === 3), '缺血时缺锦囊 3 个选项（八卦/龟背/奇门）');
    e.state.players[fp].hp = 1;
    const opts2 = e.getPouchOptions(fp);
    assert(opts2.some(o => o.pouch === PouchType.Can && o.choices.length === 5), '残血时残锦囊 5 个选项');
  }
