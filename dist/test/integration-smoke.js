...
async function once(socket, evt) {
    return new Promise((resolve) => {
        socket.once(evt, (d) => resolve(d));
    });
}
async function main() {
    ...
    try {
        ...
        const p1JoinAck = await new Promise(res => p1.emit('joinRoom', { name: '玩家A' }, (ok, data) => res({ ok, data })));
        ...
        const [p2StartEvt, p1StartEvt, p2JoinAck] = await Promise.all([
            once(p2, 'eventGameStart'),
            once(p1, 'eventGameStart'),
            new Promise(res => p2.emit('joinRoom', { name: '玩家B' }, (ok, data) => res({ ok, data }))),
        ]);
        ...
        assert(p1State.deckCount + p1State.discardCount + p1State.you.handCards.length + p2State.you.handCards.length === 107, `牌库+弃牌+双手牌 = 107 张，实际 ${p1State.deckCount + p1State.discardCount + p1State.you.handCards.length + p2State.you.handCards.length}`);
        // ---- 4. 找到当前先手玩家（谁是 activePid），由他出一张牌 ----
        const firstPid = p1State.activePid;
        ...
        const [buffEvt, bonusAck] = await Promise.all([
            once(p1, 'eventBuffChange').catch(() => null), // 任意一方都能收到广播
            new Promise(res => activeSocket.emit('useBonus', { type: 'normal' }, (ok, data) => res({ ok, data }))),
        ]);
        assert(bonusAck.ok, `${activeLabel} useBonus(type=normal) 成功：${bonusAck.data?.message || ''}`);
        // b) 找一张武将牌打出
        const generalCard = activeState.you.handCards.find((c) => c.category === 'general');
        if (!generalCard) {
            ...
            const playPromise = new Promise(res => activeSocket.emit('playCard', { cardUid: generalCard.uid }, (ok, data) => res({ ok, data })));
            const evtPromise = once(p1, 'eventPlayCard');
            const [playAck, playEvt] = await Promise.all([playPromise, evtPromise]);
            ...
            // 防御方放弃防御
            const defPid = (1 - firstPid);
            const defSocket = defPid === 0 ? p1 : p2;
            const defPromise = new Promise(res => defSocket.emit('confirmDefend', {}, (ok, data) => res({ ok, data })));
            const dmgPromise = once(p1, 'eventDamage').catch(() => null);
            const [defAck] = await Promise.all([defPromise, dmgPromise]);
        ...
        const currActive = p1State.activePid;
        const currSocket = currActive === 0 ? p1 : p2;
        const otherPid = (1 - currActive);
        const otherSocket = otherPid === 0 ? p1 : p2;
        // 第一方结束行动（操作权交给对方）
        const end1 = await new Promise(res => currSocket.emit('readyNextTurn', {}, (ok, data) => res({ ok, data })));
        ...
        // 第二方也结束行动 → 触发回合终局
        const turnEvtPromise = once(p1, 'eventTurnEnd').catch(() => null);
        const end2 = await new Promise(res => otherSocket.emit('readyNextTurn', {}, (ok, data) => res({ ok, data })));
        const [turnEvt] = await Promise.all([turnEvtPromise]);
...
//# sourceMappingURL=integration-smoke.js.map