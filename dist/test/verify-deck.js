...
function main() {
    ...
    for (const cat of Object.keys(counter)) {
        ...
        if (!ok)
            allOk = false;
        console.log(`${ok ? '✓' : '✗'} ${cards_1.CARD_COLORS[cat].label.padEnd(10)} 数量 ${String(info.count).padStart(2)} / ${info.expected} · 唯一名 ${info.names.size} 种：${[...info.names].join('、')}`);
    }
    // 武将攻击/耗气汇总
    const generals = deck.filter(c => c.category === types_1.CardCategory.General);
...
main();
//# sourceMappingURL=verify-deck.js.map