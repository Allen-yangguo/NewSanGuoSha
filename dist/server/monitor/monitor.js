...
/** 当前在线/对局快照(由采样器刷新) */
const liveNow = { online: 0, activeTables: 0 };
let sampler = null;
...
function markDirty(ts) {
    dirtyDays.add(dateStrOf(ts));
}
// ============================================================
// 工具函数
// ============================================================
function bucketKeyOf(ts) {
 ...
 */
function flushToDisk(force = false) {
    try {
...
function loadHistory() {
    ...
    try {
        ...
        for (const b of all) {
            if (b.ts > now + 5 * 60000)
                continue; // 时钟异常的未来数据跳过
            if (b.ts >= cutoff)
                buckets.push(b); // 窗口内 → 内存,重启后无缝衔接
...
function sumOfWindow(list, fromTs) {
    let requests = 0;
    let bytesOut = 0;
    let bytesIn = 0;
    for (const b of list) {
    ...
    router.get('/series', (req, res) => {
        ...
        if (useHour) {
            ...
            for (const b of windowed) {
                ...
                if (b.online > h.online)
                    h.online = b.online;
                if (b.activeTables > h.activeTables)
                    h.activeTables = b.activeTables;
...
function startMonitor(getLive) {
    ...
    const tick = () => {
        ...
        if (s.online > b.online)
            b.online = s.online;
        if (s.activeTables > b.activeTables)
            b.activeTables = s.activeTables;
    };
    tick();
