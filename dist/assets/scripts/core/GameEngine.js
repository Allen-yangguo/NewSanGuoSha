...
class GameEngine {
    ...
    playSunShangXiang(card, actor) {
        ...
        const avail = [];
        if (zhuge.que)
            avail.push(types_1.PouchType.Que);
        if (zhuge.can)
            avail.push(types_1.PouchType.Can);
        if (zhuge.ji)
            avail.push(types_1.PouchType.Ji);
        const picked = avail[Math.floor(Math.random() * avail.length)];
        zhuge[picked] = false;
        if (!me.pouches.zhuge)
            me.pouches.zhuge = { que: false, can: false, ji: false };
        me.pouches.zhuge[picked] = true;
        const name = picked === types_1.PouchType.Que ? '缺' : picked === types_1.PouchType.Can ? '残' : '急';
    ...
    settleUltimateSave(pid) {
        const res = this.ultimateSaveResult;
        if (!res)
            return { ok: false, message: '非绝杀急救结算阶段' };
        this.ultimateSaveResult = null;
        const target = this.state.players[pid];
        if (res.saved) {
            target.hp = 1;
